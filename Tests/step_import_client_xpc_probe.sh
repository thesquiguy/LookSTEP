...
products_dir="${STEPLOOK_RELEASE_PRODUCTS:-$repo_dir/build/StepLook-IncompleteNotice/Build/Products/Release}"
...
scratch_dir="$(mktemp -d "${TMPDIR:-/tmp}/step-import-client-xpc-probe.XXXXXX")"
trap 'rm -rf "$scratch_dir"' EXIT HUP INT TERM
probe_bundle="$scratch_dir/StepImportClientXPCProbe.app"
mkdir -p \
...
xcrun swiftc \
  ...
  "$repo_dir/Tests/StepImportClientXPCProbe.swift" \
  -o "$probe_bundle/Contents/MacOS/StepImportClientXPCProbe"

cp "$repo_dir/Tests/StepImportClientXPCProbe-Info.plist" \
...
"$probe_bundle/Contents/MacOS/StepImportClientXPCProbe"
